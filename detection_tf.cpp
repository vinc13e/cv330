//
// Created by vmachado on 30/11/17.
//
...
void mat2tensor(Mat & in, tensorflow::Tensor &t) {

    t = Tensor(tensorflow::DT_DOUBLE, tensorflow::TensorShape({1,in.cols,in.rows, 3})); // d <- in.deph

// get pointer to memory for that Tensor
    double *p = t.flat<double>().data();
// create a "fake" cv::Mat from it
    cv::Mat cameraImg(in.cols,in.rows, CV_32FC3, p);
//    imshow("ci", cameraImg); cv::waitKey();

// use it here as a destination
//    cv::Mat imagePixels = ...; // get data from your video pipeline
    in.convertTo(cameraImg, CV_64FC3);
    ///in = in * 255.0;
    ///t=nt;
}
...
{
    cv::CommandLineParser parser(argc, argv, params_);
    ...
    string graph = parser.get<string>("pb");
    //String modelConfiguration = parser.get<string>("pbtxt");
    string image = parser.get<String>("in");
    float confidenceThreshold = parser.get<float>("min_confidence");

    string labels = "ship, batea";//"labels/mscoco_label_map.pbtxt";
...
//    const Tensor& resized_tensor = resized_tensors[0];
    ...
    //cv::imshow("image", frame); cv::waitKey();

    mat2tensor(frame, resized_tensor);
    ...
    for(size_t i = 0; i < num_detections(0) && i < 20;++i)
    {
        if(scores(i) >= confidenceThreshold)
        {
            ...
            int xLeftBottom = static_cast<int>(boxes(0,i,1) * frame.cols);
            int yLeftBottom = static_cast<int>(boxes(0,i,0) * frame.rows);
            int xRightTop = static_cast<int>(boxes(0,i,3) * frame.cols);
            int yRightTop = static_cast<int>(boxes(0,i,2) * frame.rows);
